...
import { Plus, Edit3, Trash2, Users, CreditCard, Calendar, CheckCircle, AlertCircle } from 'lucide-react';
              ...
              {members.map((item) => (
                <tr key={item._id} className="hover:bg-violet-50/30 transition">
                  ...
                  <td className="px-6 py-4">
                    <div className="font-bold text-slate-800 leading-tight">{item.institutionName}</div>
                    <div className="text-[11px] text-violet-600 mt-1 font-medium flex items-center">
                      <span className="bg-violet-100 px-1.5 py-0.5 rounded mr-1">{item.contactPerson}</span>
                      {item.designation}
                    </div>
                  </td>
                  ...
                  {/* Membership Type */}
                  <td className="px-6 py-4">
                    <span className="text-sm text-slate-600 font-medium">{item.membershipType}</span>
                  </td>

                  {/* Subscription Types (Logic to show active ones) */}
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1">
                      {item.subscriptionTypes?.automotiveAbstracts && <Badge text="Abstracts" />}
                      {item.subscriptionTypes?.araiJournal && <Badge text="Journal" />}
                      {(!item.subscriptionTypes?.automotiveAbstracts && !item.subscriptionTypes?.araiJournal) && 
                        <span className="text-slate-400 text-xs italic">No active subs</span>
                      }
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center text-xs text-slate-500">
                      ...
                      {item.membershipStartDate ? new Date(item.membershipStartDate).toLocaleDateString() : 'N/A'}
                    </div>
                    <div className="text-[10px] text-slate-400 mt-1">
                      to {item.membershipEndDate ? new Date(item.membershipEndDate).toLocaleDateString() : 'N/A'}
                    </div>
                  </td>

                  {/* Fees & Payment Status */}
                  <td className="px-6 py-4 text-center">
                    <div className="text-sm font-bold text-slate-700">₹{item.fees || 0}</div>
                    <div className={`flex items-center justify-center text-[10px] font-bold uppercase mt-1 ${item.paymentStatus === 'Paid' ? 'text-violet-500' : 'text-violet-500'}`}>
                      {item.paymentStatus === 'Paid' ? <CheckCircle className="w-3 h-3 mr-1" /> : <AlertCircle className="w-3 h-3 mr-1" />}
